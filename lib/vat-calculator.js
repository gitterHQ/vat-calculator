...
function VatRateCalculator(supplierCountry, supplierVatNumber, options) {
  ...
  this.eservice = options && options.eservice || true;
}

VatRateCalculator.prototype.calculateVat = function(customerCountry, customerVatNumber, transactionDate, callback) {
  var self = this;

  if(typeof transactionDate === 'function') {
  ...
  return q.fcall(function() {
      ...
      // Override difference between EU and ISO 3166-1 country codes
      if (customerCountry === 'EL') {
        customerCountry = 'GR';
      } else if (customerCountry === 'UK') {
        customerCountry = 'GB';
      }
      ...
      var now = transactionDate || self._getCurrentDate();
